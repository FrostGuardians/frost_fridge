...
while True:
    ...
    if key == 'c':
        # Discard a few initial frames to clear the buffer
        for _ in range(5):
            cap.read()
        ...
        # Save the image with a unique name using precise time
        img_name = f"{save_dir}/{time.time():.6f}.png"
        ...
        # Introduce a slight delay before the next capture
        time.sleep(1)
